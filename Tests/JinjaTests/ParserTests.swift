...
struct ParserTests {
    ...
    func testArrayLiteralParsing() throws {
        ...
        if case .expression(.array(let elements)) = nodes[0] {
            ...
            if case .integer(let intVal) = elements[0] {
                #expect(intVal == 1)
            } else { Issue.record("Expected integer in array") }
            
            if case .string(let strVal) = elements[1] {
                #expect(strVal == "hello")
            } else { Issue.record("Expected string in array") }
            
            if case .boolean(let boolVal) = elements[2] {
                #expect(boolVal == true)
            } else { Issue.record("Expected boolean in array") }
        } else {
    ...
    func testObjectLiteralParsing() throws {
        ...
        if case .expression(.object(let dict)) = nodes[0] {
            ...
            if case .string(let nameVal) = dict["name"] {
                #expect(nameVal == "John")
            } else { Issue.record("Expected string value for name") }
            
            if case .integer(let ageVal) = dict["age"] {
                #expect(ageVal == 30)
            } else { Issue.record("Expected integer value for age") }
        } else {
    ...
    func testBinaryOperationParsing() throws {
        ...
        if case .expression(.binary(.add, .identifier(let left), .identifier(let right))) = addNodes[0] {
            #expect(left == "a")
            #expect(right == "b")
        } else {
        ...
        if case .expression(.binary(.equal, .identifier(let left), .identifier(let right))) = eqNodes[0] {
            #expect(left == "x")
            #expect(right == "y")
        } else {
        ...
        if case .expression(.binary(.concat, .string(let left), .string(let right))) = concatNodes[0] {
            #expect(left == "hello")
            #expect(right == " world")
        } else {
    ...
    func testMemberAccessParsing() throws {
        ...
        if case .expression(.member(.identifier(let obj), .identifier(let prop), computed: false)) = dotNodes[0] {
            #expect(obj == "object")
        ...
        if case .expression(.member(.identifier(let obj), .string(let key), computed: true)) = bracketNodes[0] {
            #expect(obj == "object")
            #expect(key == "key")
        } else {
    ...
    func testFilterParsing() throws {
        ...
        let simpleNodes = try Parser.parse(simpleTokens)
        #expect(simpleNodes.count == 1)
        if case .expression(.filter(.identifier(let value), let filterName, let args, let kwargs)) = simpleNodes[0] {
            #expect(value == "value")
        ...
        let argsNodes = try Parser.parse(argsTokens)
        #expect(argsNodes.count == 1)
        if case .expression(.filter(.identifier(let value), let filterName, let args, let kwargs)) = argsNodes[0] {
            #expect(value == "items")
        ...
        if case .expression(.filter(.filter(.identifier(let value), let firstFilter, _, _), let secondFilter, _, _)) = chainedNodes[0] {
            #expect(value == "value")
    ...
    func testTestOperationParsing() throws {
        ...
        let simpleNodes = try Parser.parse(simpleTokens)
        #expect(simpleNodes.count == 1)
        if case .expression(.test(.identifier(let value), let testName, negated: let negated)) = simpleNodes[0] {
            #expect(value == "value")
        ...
        if case .expression(.test(.identifier(let value), let testName, negated: let negated)) = negatedNodes[0] {
            #expect(value == "value")
    ...
    func testTernaryExpressionParsing() throws {
        ...
        #expect(nodes.count == 1)
        if case .expression(.ternary(.string(let value), test: .identifier(let condition), alternate: .string(let alt))) = nodes[0] {
            #expect(value == "yes")
    ...
    func testIfElifElseStatementParsing() throws {
        let tokens = try Lexer.tokenize("{% if x == 1 %}One{% elif x == 2 %}Two{% else %}Other{% endif %}")
        let nodes = try Parser.parse(tokens)
    ...
    func testForLoopParsing() throws {
        ...
        #expect(nodes.count == 1)
        if case .statement(.for(.single(let loopVar), .identifier(let iterable), let body, let elseBody, test: let test)) = nodes[0] {
            #expect(loopVar == "item")
    ...
    func testMacroDefinitionParsing() throws {
        let tokens = try Lexer.tokenize("{% macro greet(name, greeting) %}{{ greeting }} {{ name }}{% endmacro %}")
        let nodes = try Parser.parse(tokens)
        
        #expect(nodes.count == 1)
        if case .statement(.macro(let name, let params, let body)) = nodes[0] {
            #expect(name == "greet")
    ...
    func testOperatorPrecedenceParsing() throws {
        ...
        #expect(nodes.count == 1)
        if case .expression(.binary(.add, .integer(let left), .binary(.multiply, .integer(let mulLeft), .integer(let mulRight)))) = nodes[0] {
            #expect(left == 2)
            #expect(mulLeft == 3)
            #expect(mulRight == 4)
        } else {
    ...
    func testParenthesesGroupingParsing() throws {
        ...
        #expect(nodes.count == 1)
        if case .expression(.binary(.multiply, .binary(.add, .integer(let addLeft), .integer(let addRight)), .integer(let right))) = nodes[0] {
            #expect(addLeft == 2)
            #expect(addRight == 3)
            #expect(right == 4)
        } else {
    ...
    func testComplexMemberAccessParsing() throws {
        ...
        #expect(nodes.count == 1)
        if case .expression(.member(.member(.identifier(let obj1), .identifier(let prop1), computed: false), .identifier(let prop2), computed: false)) = nodes[0] {
            #expect(obj1 == "user")
    ...
    func testMixedMemberAccessParsing() throws {
        ...
        if case .expression(.member(.member(.member(.identifier(let obj), .identifier(let prop1), computed: false), .integer(let index), computed: true), .identifier(let prop2), computed: false)) = nodes[0] {
            #expect(obj == "data")
            #expect(prop1 == "items")
            #expect(index == 0)
            #expect(prop2 == "name")
    ...
    func testInvalidExpressionSyntax() throws {
        #expect(throws: JinjaError.self) {
            _ = try Parser.parse([Token(kind: .expression, value: "invalid syntax !!!", position: 0)])
        }
    }
    ...
    func testNestedExpressionsInText() throws {
        ...
        if case .text(let text1) = nodes[0] {
            #expect(text1 == "Hello ")
        } else { Issue.record("Expected first text node") }
        
        if case .expression(.identifier(let name)) = nodes[1] {
            #expect(name == "name")
        } else { Issue.record("Expected name expression") }
        
        if case .text(let text2) = nodes[2] {
            #expect(text2 == ", you have ")
        } else { Issue.record("Expected second text node") }
        
        if case .expression(.identifier(let count)) = nodes[3] {
            #expect(count == "count")
        } else { Issue.record("Expected count expression") }
    }
    ...
    func testComplexNestedStructures() throws {
        ...
        if case .statement(.for(.single(let loopVar), .identifier(let iterable), let body, _, test: _)) = nodes[0] {
            #expect(loopVar == "user")
